...
class HorseshoeConvLayer(nn.Module):
    ...
    def log_prior(self):
        ...
        shape = self.prior_v_shape.to(device)
        exp_theta_inverse = self.theta.exp_inverse().to(device)
        exp_log_theta = self.theta.exp_log().to(device)
    ...
    def analytic_update(self):
        ...
        new_shape = torch.Tensor([1]).to(self.get_device())
    ...
    def get_device(self):
        return self.beta.mean.device

    def update_device(self):
        device = self.get_device()
        self.prior_lambda_rate = self.prior_lambda_rate.to(device)
        self.prior_theta_rate = self.prior_theta_rate.to(device)

        self.lambda_.rate = self.lambda_.rate.to(device)
        self.lambda_.shape = self.lambda_.shape.to(device)

        self.theta.rate = self.theta.rate.to(device)
        self.theta.shape = self.theta.shape.to(device)