...
from BNN.models import ABMIL, DSMIL, TransMIL, BClassifier
            ...
            for n_test in n_sample_test:
                for kl in weight_kl:
                    for shrinkage_rate in shrinkage:
                        ...
                        if args.early_stop:
                            print('early stopping mode......')
                            best_acc = 0
                            count = 0
                            while count<args.early_stop:
                                print(f'Early Stop [{count}/{args.early_stop}]......')
                                train_loss_bag = train(train_loader, milnet, criterion, optimizer, args, n_train,
                                                       weight_kl=kl)
                                if args.num_classes == 1:
                                    ...
                                    if args.wandb:
                                        wandb.log(
                                            {'train_loss': train_loss_bag, 'accuracy': accuracy, 'f1': f1, 'auc': auc})
