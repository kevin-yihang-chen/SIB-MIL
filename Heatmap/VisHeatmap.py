...
def get_magnified_image(wsi_name,level):
    ...
    wsi_path = f'/home/yhchen/Documents/data1/public/WSI/Camelyon16/CAMELYON16/{wsi_name}.tif'
...
def visualize(node_feat_mask, wsi_name, patches_coords, poly_coords, img, level, args):
    ...
    for idx, (bbox, cl) in enumerate(zip(patches_coords, colours)):
        ...
        img = cv2.rectangle(img, (int(bbox[0]), bbox[1]), (int(bbox[0]+ s / 2 ** level), int(bbox[1] + s / 2 ** level)), cl,
                            cv2.FILLED)
    ...
    output_name = os.path.join( f'./{args.model}/heatmap', wsi_name + ".jpeg")
...
def main():
    ...
    parser.add_argument('--dataset', default='Camelyon', type=str, help='Dataset folder name')
    parser.add_argument('--extractor', default='dsmil', type=str,help='MIL model')
    parser.add_argument('--model', default='abmil', type=str, help='MIL model')
    args = parser.parse_args()

    level = 5
    annotated_slides = glob.glob('/home/yhchen/Documents/data1/public/WSI/Camelyon16/lesion_annotation/*.xml')

    for slide in annotated_slides:
        wsi_name = slide.split('/')[-1][:8]
        img, wsi = get_magnified_image(wsi_name,level)
        coor_feat_mask = pd.read_csv(f'../Feats/Camelyon/simclr_files_256_v2/testing/{wsi_name}/DP_insclassifier_prob_20.csv')
        ...
        node_feat_mask = np.array(list(coor_feat_mask['prob']),dtype=float)
        node_feat_mask = sigmoid(node_feat_mask)
